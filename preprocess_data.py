...
def main():
    ...
    test(config)
    train(config)
    # list_data = [config['preprocess_train'], config['preprocess_val'], config['preprocess_test']]
    # name_data = ["train_crs_dist6_angle90.p", "val_crs_dist6_angle90.p", "test_test.p"]
    # for i in range(3):
    #     mia_path = os.path.join(
    #         root_path, "dataset", "preprocess", "mia_"+name_data[i]
    #     )
    #     pit_path = os.path.join(
    #         root_path, "dataset", "preprocess", "pit_"+name_data[i]
    #     )
    #     list_all = np.load(list_data[i], allow_pickle=True)
    #     list_mia = []
    #     list_pit = []
    #     for i in range(len(list_all)):
    #         if list_all[i]['city'] == 'PIT':
    #             list_pit.append(list_all[i])
    #         if list_all[i]['city'] == 'MIA':
    #             list_mia.append(list_all[i])
    #     f = open(mia_path, 'wb')
...
def modify(config, data_loader, save):
    ...
    f = open(os.path.join(root_path, 'preprocess', 'mia'+save), 'wb')
    pickle.dump(list_mia, f, protocol=pickle.HIGHEST_PROTOCOL)
    f.close()

    f = open(os.path.join(root_path, 'preprocess', 'pit'+save), 'wb')
    pickle.dump(list_pit, f, protocol=pickle.HIGHEST_PROTOCOL)
    f.close()
